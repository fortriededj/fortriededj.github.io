...
function showGraph(row,low,high){
	this.event.stopPropagation();
	rowno = row.parentNode.parentNode.getAttribute('id').substring(4);
	testname = row.parentNode.parentNode.children[0].innerText;
	alert(rowno + ' Low: ' + low + ' High: ' + high + ' Name: ' + testname);
	console.log('Value = ' + CELLS[rowno][0].innerText);
	minmaxArr = MinMax(CELLS[rowno]);
	console.log('Min = ' + minmaxArr[0]);
	console.log('Max = ' + minmaxArr[1]);
